...
namespace ScintillaNET
{
    public partial class FindReplaceDialog : Form
    {
        ...
        private void cboFind_TextChanged(object sender, EventArgs e)
        {
            if (sender == cboFindF)
            {
                btnFindAll.Enabled = btnFindPrevious.Enabled = btnFindNext.Enabled = ((ComboBox)sender).Text.Length != 0;
            }
            else if (sender == cboFindR)
            {
                btnReplaceAll.Enabled = btnReplacePrevious.Enabled = btnReplaceNext.Enabled = ((ComboBox)sender).Text.Length != 0;
            }
        }
