...
namespace ScintillaNet
{
	public static class Utilities
	{
        ...
        /// <summary>
        /// A reverse Lexer list, this allows us to look up lexers by id
        /// </summary>
        public static Lexer LexerLookupByID(int LexerID)
        {
            Array genArrayOfIds = Enum.GetValues(typeof(Lexer));
            List<int> ListOfIds = new List<int>();
            foreach(Lexer lex in genArrayOfIds)
            {
                ListOfIds.Add((int)lex);
            }
            int currentIndex = 0;
            foreach (int id in ListOfIds)
            {
                if (id == LexerID)
                {
                    return ((Lexer)genArrayOfIds.GetValue(currentIndex));
                }
                currentIndex++;
            }
            throw new Exception("Specified Lexer Not Found!");
        }
