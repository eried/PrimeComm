...
namespace ScintillaNet
{
    ...
    public partial class Scintilla : Control, INativeScintilla, ISupportInitialize
    {
        ...
        public const string DefaultDllName64 = "SciLexerx64.dll";
        ...
        protected override CreateParams CreateParams
        {
            ...
            get
            {
                ...
                if (!_sciLexerLoaded)
                {
                    //  If run on 64bit load the correct unmnanaged SciLexerx64.dll
                    //  Note that Visual Studio itself as of 2010 is still 32bit
                    //  so it should be loading the 32bit version for the Windows
                    //  forms designer.
                    if (Marshal.SizeOf(IntPtr.Zero) == 8)
                    {
                        try
                        {
                            LoadSciLexer(DefaultDllName64);
                        }
                        catch (FileNotFoundException)
                        {
                            //  Try to fall back on 32bit version of SciLexer.dll
                            //  If this fails exception will bubble up to the app
                            //  same as it did before
                            LoadSciLexer(DefaultDllName);
                        }
                    }
                    else
                    {
        ...
        /// <summary>
        /// Loads the unmanaged SciLexer module so that it's available for PInvoke
        /// </summary>
        private void LoadSciLexer(string dllName)
        {
            if (NativeMethods.LoadLibrary(dllName) == IntPtr.Zero)
            {
        ...
        protected virtual void OnAutoCompleteAccepted(AutoCompleteAcceptedEventArgs e)
        {
            ...
            if (handler != null)
            {
                e.Text = AutoComplete.SelectedText;
                handler(this, e);
            }
