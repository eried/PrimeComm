...
namespace ScintillaNet.Configuration
{
	...
	public class ConfigurationManager : TopLevelHelper
	{
		...
		internal void Configure(List<Configuration> configList)
		{
			...
			foreach (Configuration c in configList)
			{
				if (c.AutoComplete_CancelAtShowPoint.HasValue)
					b = c.AutoComplete_CancelAtShowPoint;
			}
			if (b.HasValue)
				Scintilla.AutoComplete.CancelAtShowPoint = b.Value;
            ...
            foreach (Configuration c in configList)
            {
                if (c.AutoComplete_TriggerStrings.Count > 0)
                    ls = c.AutoComplete_TriggerStrings;
            }
            if (ls.Count > 0)
                Scintilla.AutoComplete.TriggerChars = ls;
			...
			sl.Sort();
            foreach (Snippet sc in Scintilla.Snippets.List)
            {
                Scintilla.AutoComplete.List.Add(sc.Shortcut + Scintilla.AutoComplete.ImageSeparator + AutoComplete.SnippetImageIndex.ToString());
            }
