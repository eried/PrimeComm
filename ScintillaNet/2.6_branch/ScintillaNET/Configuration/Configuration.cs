	...
	public class Configuration
	{
        ...
        public void Load(XmlDocument configDocument)
        {
            ...
            if (autoCNode != null)
            {
                ...
                _autoComplete_cancelAtShowPoint = getBool(autoCNode.GetAttribute("CancelAtShowPoint"));
                ...
                XmlNodeList Nodelist = autoCNode.SelectNodes("./List") as XmlNodeList;
                if (Nodelist != null)
                {
                	foreach(XmlElement listNode in Nodelist)
                	{
                        _autoComplete_ListInherit = getBool(listNode.GetAttribute("Inherit"));
                		if (listNode.GetAttribute("Icon") != "")
                		{
                			string icoString = listNode.GetAttribute("Icon");
                			StringBuilder tmp = new StringBuilder();
                			foreach (string str in new Regex("\\s+").Replace(listNode.InnerText, " ").Trim().Split(new char[] { (AutoComplete_ListSeperator.HasValue) ? AutoComplete_ListSeperator.Value : ' ' }))
                			{
                                tmp.Append(str + ((AutoComplete_ImageSeperator.HasValue) ? new string(new char[] { AutoComplete_ImageSeperator.Value }) : "?") + icoString + ((AutoComplete_ListSeperator.HasValue) ? new string(new char[] { AutoComplete_ListSeperator.Value }) : " "));
                			}
                			_autoComplete_List = tmp.ToString();
                            //throw new Exception();
                		}
                		else
                		{
		...
		private void readAutoComplete(XmlReader reader)
		{
				...
				while (reader.MoveToNextAttribute())
				{
					...
					switch (attrName)
					{
						...
						case "automaticlengthentered":
							...
							break;
						case "cancelatstart":
							_autoComplete_cancelAtShowPoint = getBool(reader.Value);
							break;
						case "droprestofword":
			...
			if (!reader.IsEmptyElement)
			{
				while (!(reader.NodeType == XmlNodeType.EndElement && reader.Name.Equals("autocomplete", StringComparison.OrdinalIgnoreCase)))
				{
                    ...
                    if (reader.NodeType == XmlNodeType.Element && reader.Name.Equals("list", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (reader.HasAttributes)
                        {
                            while (reader.MoveToNextAttribute())
                            {
                                ...
                                else if (reader.Name.Equals("icon", StringComparison.OrdinalIgnoreCase))
                                {
                                    icoString = reader.Value;
                                }
                            }
                            reader.MoveToElement();
                        }
                        ...
                        {
                            StringBuilder tmp = new StringBuilder();
                            foreach (string str in new Regex("\\s+").Replace(reader.ReadString(), " ").Trim().Split(new char[] { (AutoComplete_ListSeperator.HasValue) ? AutoComplete_ListSeperator.Value : ' ' }))
                            {
                                tmp.Append(str + ((AutoComplete_ImageSeperator.HasValue) ? new string(new char[] { AutoComplete_ImageSeperator.Value }) : "?") + icoString + ((AutoComplete_ListSeperator.HasValue) ? new string(new char[] { AutoComplete_ListSeperator.Value }) : " "));
                            }
                            _autoComplete_List = ( _autoComplete_List + ((AutoComplete_ListSeperator.HasValue) ? new string(new char[] { AutoComplete_ListSeperator.Value }) : " ") + tmp.ToString()).Trim();
                        }
                        else
                        {
                    ...
                    if (reader.NodeType == XmlNodeType.Element && reader.Name.Equals("triggerstring", StringComparison.OrdinalIgnoreCase))
                    {
                        if (reader.HasAttributes)
                        {
                            while (reader.MoveToNextAttribute())
                            {
                                ...
                                {
                                    _autoComplete_triggerStrings.Add(reader.Value);
                                }
                            }
                            reader.MoveToElement();
                        }
		...
		private bool? _autoComplete_cancelAtShowPoint;
		public bool? AutoComplete_CancelAtShowPoint
		{
			get
			{
				return _autoComplete_cancelAtShowPoint;
			}
			set
			{
				_autoComplete_cancelAtShowPoint = value;
			}
		}
        ...
        private List<String> _autoComplete_triggerStrings = new List<String>();
        public List<String> AutoComplete_TriggerStrings
        {
            get
            {
                return _autoComplete_triggerStrings;
            }
            set
            {
                _autoComplete_triggerStrings = value;
            }
        }
